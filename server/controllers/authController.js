...
exports.signUp = catchAsync(async (req, res, next) => {
  ...
  newAccount.activeAccountTokenExpires =
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  //   new AppError("Votre compte a été supprimer.", 400);
  // }
...
exports.resetPasswordStepOne = catchAsync(async (req, res, next) => {
  ...
  if (!account) {
    return next(
      new AppError("Le Code est invalide ou a expiré !! ", 400)
    );
  }
...
exports.resetPasswordStepTwo = catchAsync(async (req, res, next) => {
  ...
  if (req.body.password.length < 8) {
    return next(
      new AppError(
        "Les mots de passe doit contenir au moins 8 caractères. ",
        400
      )
    );
  }
